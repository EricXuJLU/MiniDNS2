...
func GRPCClient(addr string) {
	...
	client := &gRPCClient{}
	var err error
	client.conn, err = grpc.Dial(addr,
		grpc.WithInsecure(),
		grpc.WithUnaryInterceptor(grpc_middleware.ChainUnaryClient(service.UnaryClientInterceptor)))
...
func (c gRPCClient) handle() {
	...
	case "get":
		handleGet(c.Client, orders[1])
	case "insert":
		handleInsert(c.Client, orders[1], orders[2])
	case "delete":
		handleDelete(c.Client, orders[1], orders[2])
	case "update":
		handleUpdate(c.Client, orders[1], orders[2], orders[3], orders[4])
	default:
